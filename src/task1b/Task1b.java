...
public class Task1b {
	...
	public static class Generator<T> extends GenRunnable<T> {
		...
		public void prepareInput(CompEnv<T> gen) {
			...
			System.out.println("Case"+caseSta[0].numOfG1G1+"\t"+caseSta[0].numOfG1G2+"\t"+caseSta[0].numOfG2G2);
			System.out.println("Control"+controlSta[0].numOfG1G1+"\t"+controlSta[0].numOfG1G2+"\t"+controlSta[0].numOfG2G2);
	...
	public static class Evaluator<T> extends EvaRunnable<T> {
		...
		public void secureCompute(CompEnv<T> gen) {
			res = gen.newTArray(numOfTests, 0);
			IntegerLib<T> lib = new IntegerLib<T>(gen);
			flib = new FixedPointLib<T>(gen, FWidth, FOffset);//10e-6
			// TODO Auto-generated method stub
			for(int i = 0; i < numOfTests; ++i) {
				T[][] caseFre = gen.newTArray(3, 0);
				T[][] controlFre = gen.newTArray(3, 0);
				...
				for(int k = 0; k < 3; ++k) {
					...
					T[] sq2 = lib.multiplyFull(controlFre[0], controlFre[0]);
				...
				resCase = flib.div(resCase, flib.publicValue(1.0/200));
				resControl = flib.div(resControl, flib.publicValue(1.0/200));
				res[i] = flib.add(resCase, resControl);
			}
		}
