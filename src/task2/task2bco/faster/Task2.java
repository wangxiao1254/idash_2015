...
import task2.task2bco.PrepareData;
import task2.task2bco.SNPEntry;
...
public class Task2 {
	...
	public static class Generator<T> extends GenRunnable<T> {
		...
		public void prepareInput(CompEnv<T> gen) {
			...
			System.out.println(bf2.size() +"   "+ alicelength);
		...
		public void secureCompute(CompEnv<T> gen) {
			...
			double r = Utils.toBigInteger(gen.outputToAlice(res)).doubleValue();
System.out.println("!!"+bf.countToSize((int) r));
			res2 = compute(gen, aliceBF2, bobBF2);
			r = Utils.toBigInteger(gen.outputToAlice(res2)).doubleValue();
System.out.println("!!"+bf.countToSize((int) r));
			...
			System.out.println(Utils.toInt(gen.outputToAlice(m_X)));
			System.out.println(Utils.toInt(gen.outputToAlice(m_Y)));

			int length = 2*Math.max(m_X.length, m_Y.length);
...
//			System.out.println(2*r- totalSize);
			System.out.println(r+" "+bf.m+" "+bf.k+" "+totalSize);
			double size = -1*bf.m / bf.k * Math.log(r/bf.m/bf.m)-totalSize;
	...
	public static class Evaluator<T> extends EvaRunnable<T> {
		...
		public void secureCompute(CompEnv<T> gen) {
			...
			IntegerLib<T> lib = new IntegerLib<T>(gen); 
//			gen.outputToAlice(res);gen.outputToAlice(res2);
			T[] m_X = lib.sub(lib.toSignals(bf.m, res.length), res);
			T[] m_Y = lib.sub(lib.toSignals(bf.m, res2.length), res2);
			gen.outputToAlice(m_X);gen.outputToAlice(m_Y);
