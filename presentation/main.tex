...
\begin{frame}{Building Block: Secure Set Union}
  \myblock{10}{0}{0}{Alice\\
    ...
    \begin{algorithm}[H]
      \begin{algorithmic}[1]
        \STATE {\color{red} Sort the input array $d[]$ obliviously}
...
\begin{frame}{Set Union: Oblivious Merge}
  \begin{itemize}
    \item {\color{green} Local sort of $S^A$ and $S^B$}
    \item {\color{red} Oblivious merge of sorted lists $(O(n \log n))$}
    \item {\color{red} Compute set union in a single pass}
  \end{itemize}
\end{frame}
