...
where each $e^A_i, e'^A_i$ is from $\{A, T, C, G\}$.\\
Input for Bob: two lists of alleles $l^B_{case} = (e^B_1,...,e^B_n)$, $l^B_{control} = (e'^B_1,...,e'^B_n)$.\\
where each $e^B_i, e'^B_i$ is from $\{A, T, C, G\}$.\\

First let us define $a, b$ be the frequency of two types of alleles in
$l_{case} = l^A_{case} || l^B_{case}$ and $c, d$ be the frequency of two types of alleles in
