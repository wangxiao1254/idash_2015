...
Before going into how to solve Task2, we first present the solution to estimate the size of union of two sets.
...
A straightforward way of calculating the union is to use oblivious sorting, as detailed in Algorithm~\ref{alg1}.
...
In order to compute {\tt d1}, we construct a new set: for every record $(pos, val)$, each party insert $(pos, i), i\in[1, len(val)]$ to the set and get set $S_1^a, S_1^b$. Then it 
