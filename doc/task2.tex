...
This strawman appraoch involves an oblivious sorting on the input data, which requires a circuit of size $O(Dn\log^2n)$ circuit, 
using bitonic sorting network~\cite{bitonicsort}. In our implementation of this approach,  we utilize the local computation:
...
only need a circuit of size $O(Dn\log n)$. The {\tt ObliVM-lang} code for this approach is presented in Figure~\ref{fig:obl_merge}.
