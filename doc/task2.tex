...
The following table gives typical parameter choices (for values of 
$N$ given in the example dataset)
...
\end{tabular}
\end{center}
\caption{Parameters and accuracy for bloom filter cardinality estimation.
...
\begin{table}[t]
  \centering
  \begin{tabular}{c|c}
    Failure probability & Number of bits for hashing \\ \hline
    $2^{-10}$ & 43 \\
    $2^{-20}$ & 53 \\
    $2^{-30}$ & 63 \\
    $2^{-40}$ & 73 \\
